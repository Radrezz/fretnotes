...
<body class="bg-cream text-charcoal font-sans min-h-screen flex flex-col">
  ...
  <main class="max-w-6xl mx-auto px-6 py-10 flex-grow">
    ...
    <section class="mb-12">
      ...
      <form method="POST" class="flex flex-wrap gap-3 bg-purewhite p-4 rounded-lg border border-beige mb-6 shadow">
        ...
        <button type="submit" name="add_song" class="bg-terracotta text-purewhite px-4 py-2 rounded hover:bg-[#9e6047] transition">Add Song</button>
      </form>
          ...
          <?php foreach ($songs as $s): ?>
            <tr class="border-b border-beige hover:bg-cream">
              ...
              <td class="p-3 space-x-2">
                <button onclick="openEditSongModal(<?php echo htmlspecialchars(json_encode($s)); ?>)" class="text-blue-600 hover:underline">Edit</button>
                <a href="?delete_song=<?php echo $s['id']; ?>" onclick="return confirm('Delete this song?');" class="text-red-600 hover:underline">Delete</a>
              </td>
            </tr>
          <?php endforeach; ?>
        </tbody>
    ...
    <section class="mb-12">
      ...
      <form method="POST" class="flex flex-wrap gap-3 bg-purewhite p-4 rounded-lg border border-beige mb-6 shadow">
        ...
        <button type="submit" name="add_user" class="bg-terracotta text-purewhite px-4 py-2 rounded hover:bg-[#9e6047] transition">Add User</button>
      </form>
        ...
        <tbody>
          <?php foreach ($users as $u): ?>
            <tr class="border-b border-beige hover:bg-cream">
              ...
              <td class="p-3 space-x-2">
                <button onclick="openEditUserModal(<?php echo htmlspecialchars(json_encode($u)); ?>)" class="text-blue-600 hover:underline">Edit</button>
                <a href="?delete_user=<?php echo $u['id']; ?>" onclick="return confirm('Delete this user?');" class="text-red-600 hover:underline">Delete</a>
              </td>
            </tr>
          <?php endforeach; ?>
        </tbody>
        ...
        <tbody>
          <?php foreach ($threads as $t): ?>
            <tr class="border-b border-beige hover:bg-cream">
              ...
              <td class="p-3">
                <a href="?delete_thread=<?php echo $t['id']; ?>" onclick="return confirm('Delete this thread?');" class="text-red-600 hover:underline">Delete</a>
              </td>
            </tr>
          <?php endforeach; ?>
        </tbody>
  ...
  <!-- FOOTER -->
  <footer class="bg-charcoal text-purewhite py-6 text-center">
    &copy; 2025 FretNotes Admin Panel. All Rights Reserved.
  </footer>
  ...
  <div id="editSongModal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center">
    <form method="POST" class="bg-purewhite p-6 rounded-lg shadow-lg w-96">
      ...
      <input name="title" id="editSongTitle" placeholder="Title" required class="border border-beige p-2 rounded w-full mb-3">
      <input name="artist" id="editSongArtist" placeholder="Artist" required class="border border-beige p-2 rounded w-full mb-3">
      <input name="genre" id="editSongGenre" placeholder="Genre" required class="border border-beige p-2 rounded w-full mb-3">
      <input name="version_name" id="editSongVersion" placeholder="Version" required class="border border-beige p-2 rounded w-full mb-3">
      <div class="flex justify-end gap-2">
        <button type="button" onclick="closeModal('editSongModal')" class="px-3 py-2 border border-beige rounded">Cancel</button>
  ...
  <div id="editUserModal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center">
    <form method="POST" class="bg-purewhite p-6 rounded-lg shadow-lg w-96">
      ...
      <input name="username" id="editUserUsername" placeholder="Username" required class="border border-beige p-2 rounded w-full mb-3">
      <input name="password" id="editUserPassword" type="password" placeholder="New Password (optional)" class="border border-beige p-2 rounded w-full mb-3">
      <select name="role" id="editUserRole" class="border border-beige p-2 rounded w-full mb-3">
        <option value="user">User</option>
        <option value="admin">Admin</option>
      </select>
      <div class="flex justify-end gap-2">
        <button type="button" onclick="closeModal('editUserModal')" class="px-3 py-2 border border-beige rounded">Cancel</button>
